...
then
    ...
    oci bastion bastion create --bastion-type "standard" -c $subnet_compartment_id --target-subnet-id $subnet_id --name "QuickBastion$subnet_name" --client-cidr-list $proxy_list  --wait-for-state "SUCCEEDED" | jq -r '.data.id'
    bastion_id=$(oci bastion bastion list -c $subnet_compartment_id --all | jq -r --arg SUBNET_ID "$subnet_id" 'first(.data[]|select(."lifecycle-state" == "ACTIVE" and ."target-subnet-id" == $SUBNET_ID)|.id)')
fi
...
bastion_user_name=$(oci bastion session get --session-id $session_id | jq -r '.data."bastion-user-name"')
